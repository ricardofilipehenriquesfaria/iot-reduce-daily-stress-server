...
function createNewRequest(mysql_module, nodemailer_module){ 
	xmlReader.readXML(fs.readFileSync(FILE), function(err, data) {
		...
		for(var i = 0; i < result.kml.Document.Folder[0].Placemark.length; i++){
			...
			for(var j = 0; j < result.kml.Document.Folder[0].Placemark[i].ExtendedData.Data.length; j++){
				...
				if(equals(result.kml.Document.Folder[0].Placemark[i].ExtendedData.Data[j]._attributes.name, "Localização")){
				...
				if(equals(result.kml.Document.Folder[0].Placemark[i].ExtendedData.Data[j]._attributes.name, "Justificação")){
